...
import { KnowledgeType, RelationshipType } from '../../../services/workspace';
  ...
  public setupHandlers(socket: any, connectionInfo: ConnectionInfo): void {
    ...
    socket.on('workspace:join', async (data: { workspaceId: string }) => {
      try {
        ...
        await this.services.workspaceService.sync.subscribeToWorkspace(
          workspaceId,
          connectionInfo.userId || socket.id,
          (event) => {
        ...
        if (connectionInfo.userId) {
          await this.services.workspaceService.sync.updateUserPresence(
            workspaceId,
    ...
    socket.on('workspace:leave', async (data: { workspaceId: string }) => {
      try {
        ...
        if (connectionInfo.userId) {
          await this.services.workspaceService.sync.unsubscribeFromWorkspace(
            workspaceId,
            connectionInfo.userId
          );
        ...
        logger.error('Workspace leave error:', error);
    ...
    socket.on('workspace:cursor_update', async (data: { 
      ...
      try {
        ...
        await this.services.workspaceService.sync.updateCursorPosition(
          workspaceId,
          connectionInfo.userId,
          position
        );
    ...
    socket.on('workspace:add_knowledge', async (data: {
      ...
      try {
        ...
        // Add knowledge to workspace
        const knowledgeNode = await this.services.workspaceService.manager.addKnowledge(
          workspaceId,
          {
            ...knowledge,
            metadata: {
    ...
    socket.on('workspace:search_knowledge', async (data: {
      ...
      try {
        ...
        const results = await this.services.workspaceService.manager.queryKnowledge(
          workspaceId,
          query,
          limit
        );
    ...
    socket.on('workspace:add_relationship', async (data: {
      ...
      try {
        ...
        await this.services.workspaceService.manager.addKnowledgeRelationship(
          workspaceId,
          sourceId,
          targetId,
          relationship,
    ...
    socket.on('workspace:get_insights', async (data: { workspaceId: string }) => {
      try {
        const { workspaceId } = data;
    ...
    socket.on('workspace:get_metrics', async (data: { workspaceId: string }) => {
      try {
        const { workspaceId } = data;
    ...
    socket.on('workspace:acquire_lock', async (data: {
      ...
      try {
        ...
        const acquired = await this.services.workspaceService.sync.acquireLock(
          workspaceId,
          resourceId,
          connectionInfo.userId,
          lockType
    ...
    socket.on('workspace:release_lock', async (data: {
      ...
      try {
        ...
        await this.services.workspaceService.sync.releaseLock(
          workspaceId,
          resourceId,
          connectionInfo.userId
        );
