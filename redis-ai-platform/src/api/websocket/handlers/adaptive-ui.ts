...
export class AdaptiveUIHandler {
  ...
  public setupHandlers(socket: any, connectionInfo: ConnectionInfo): void {
    ...
    socket.on('adaptive_ui:track_interaction', async (data: {
      ...
      try {
        ...
        socket.emit('error', { message: 'Failed to track interaction', error: (error as Error).message });
      }
    });
    ...
    socket.on('adaptive_ui:subscribe_personalization', async () => {
      try {
        ...
        const suggestions = await this.services.adaptiveUIService.uiPersonalizer.generatePersonalizationSuggestions(
        ...
        logger.error('Subscribe personalization error:', error);
        socket.emit('error', { message: 'Failed to subscribe to personalization', error: (error as Error).message });
      }
    });
    ...
    socket.on('adaptive_ui:request_adaptation', async (data: {
      ...
      try {
        ...
        const result = await this.services.adaptiveUIService.adaptiveUIController.processAdaptationRequest(
          adaptationRequest
        );
        ...
        logger.error('Request adaptation error:', error);
        socket.emit('error', { message: 'Failed to process adaptation request', error: (error as Error).message });
      }
    });
    ...
    socket.on('adaptive_ui:get_assistance', async (data: {
      ...
      try {
        ...
        const help = await this.services.adaptiveUIService.contextualAssistant.provideContextualHelp(
          assistanceContext
        );
        ...
        logger.error('Get assistance error:', error);
        socket.emit('error', { message: 'Failed to get assistance', error: (error as Error).message });
      }
    });
    ...
    socket.on('adaptive_ui:check_feature_readiness', async (data: {
      ...
      try {
        ...
        const patterns = await this.services.adaptiveUIService.interactionTracker.getUsagePatterns(connectionInfo.userId);
        const readiness = await this.services.adaptiveUIService.featureIntroducer.assessFeatureReadiness(
          connectionInfo.userId,
          featureId,
          patterns
        );
        ...
        if (readiness.ready && readiness.confidence > 0.8) {
          ...
          const introduction = await this.services.adaptiveUIService.featureIntroducer.introduceFeature(
            featureId,
            introductionContext
          );
        ...
        logger.error('Check feature readiness error:', error);
        socket.emit('error', { message: 'Failed to check feature readiness', error: (error as Error).message });
      }
    });
    ...
    socket.on('adaptive_ui:update_accessibility', async (data: {
      ...
      try {
        ...
        const profile = await this.services.adaptiveUIService.accessibilityAdapter.createAccessibilityProfile(
          connectionInfo.userId,
          { accessibility: preferences } as any
        );

        // Generate accessibility adaptations
        const adaptations = await this.services.adaptiveUIService.accessibilityAdapter.generateAdaptations(
          connectionInfo.userId,
          profile
        );
        ...
        socket.emit('error', { message: 'Failed to update accessibility', error: (error as Error).message });
      }
    });
  }

  private async checkForAdaptations(socket: any, connectionInfo: ConnectionInfo, interaction: any): Promise<void> {
    try {
      // Check if user is struggling (multiple failed interactions)
      const recentInteractions = await this.services.adaptiveUIService.interactionTracker.getUserInteractions(
        connectionInfo.userId!
      );
      ...
      if (recentFailures >= 3) {
        ...
        const struggles = await this.services.adaptiveUIService.contextualAssistant.detectUserStruggles(
          connectionInfo.userId!
        );
      ...
      const patterns = await this.services.adaptiveUIService.interactionTracker.getUsagePatterns(
        connectionInfo.userId!
      );
      ...
      logger.error('Check for adaptations error:', error);
    }
  }
}