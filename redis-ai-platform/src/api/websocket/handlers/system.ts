...
export class SystemHandler {
  ...
  public setupHandlers(socket: any, connectionInfo: ConnectionInfo): void {
    ...
    socket.on('system:subscribe_alerts', async (data: { alertTypes?: string[] }) => {
      try {
        ...
        logger.error('Subscribe alerts error:', error);
        socket.emit('error', { message: 'Failed to subscribe to alerts', error: (error as Error).message });
      }
    });
    ...
    socket.on('system:unsubscribe_alerts', async () => {
      try {
        ...
        logger.error('Unsubscribe alerts error:', error);
        socket.emit('error', { message: 'Failed to unsubscribe from alerts', error: (error as Error).message });
      }
    });
    ...
    socket.on('system:subscribe_metrics', async (data: { 
      ...
      try {
        ...
        await this.redis.hset(
          `system_metrics_prefs:${socket.id}`,
          'interval',
          interval.toString(),
          'metrics',
        ...
        logger.error('Subscribe metrics error:', error);
        socket.emit('error', { message: 'Failed to subscribe to metrics', error: (error as Error).message });
      }
    });
    ...
    socket.on('system:get_health', async () => {
      try {
        ...
        socket.emit('error', { message: 'Failed to get system health', error: (error as Error).message });
      }
    });
    ...
    socket.on('system:get_stats', async () => {
      try {
        ...
        socket.emit('error', { message: 'Failed to get system stats', error: (error as Error).message });
      }
    });
    ...
    socket.on('system:report_issue', async (data: {
      ...
      try {
        ...
        logger.warn(`System issue reported: ${type} (${severity})`, issue);

      } catch (error) {
        logger.error('Report issue error:', error);
        socket.emit('error', { message: 'Failed to report issue', error: (error as Error).message });
      }
    });

    // Get recent issues
    socket.on('system:get_issues', async (data: { limit?: number; severity?: string }) => {
      try {
        ...
        socket.emit('error', { message: 'Failed to get issues', error: (error as Error).message });
      }
    });
  }
  ...
  private async checkSystemAlerts(): Promise<void> {
    try {
      ...
      if (health.overall !== 'healthy') {
        this.io.to('system:alerts').emit('system:health_alert', {
          health: {
            ...
            failedChecks: health.checks.filter((check: any) => check.status === 'fail')
          },
          timestamp: Date.now()
        });
      }
      ...
      logger.error('Check system alerts error:', error);
    }
  }
