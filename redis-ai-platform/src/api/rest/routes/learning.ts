...
export function learningRoutes(services: APIServices): Router {
  const router = Router();
  ...
  router.get('/patterns/:userId', async (req: Request, res: Response) => {
    try {
      ...
      const patterns = await services.learningService.patternAnalyzer.analyzeUserPatterns(
        userId,
        timeframe as string
      );
      ...
      res.json({
        userId,
        timeframe,
        patterns: patterns.slice(0, parseInt(limit as string)).map(pattern => ({
          id: pattern.id,
          type: pattern.type,
  ...
  router.get('/recommendations/:userId', async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        recommendations: recommendations.slice(0, parseInt(limit as string)).map(rec => ({
          id: rec.id,
          type: rec.type,
          title: rec.title,
  ...
  router.get('/similar-users/:userId', async (req: Request, res: Response) => {
    try {
      const { userId } = req.params;
      ...
      res.json({
        ...
        similarUsers: similarUsers.slice(0, parseInt(limit as string)).map(user => ({
          userId: user.userId,
  ...
  router.get('/export/:userId', async (req: Request, res: Response) => {
    try {
      ...
      if (format === 'csv') {
        ...
        const csv = Object.entries(userData).map(([key, value]) => 
