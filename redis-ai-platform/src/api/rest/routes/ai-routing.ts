...
export function aiRoutingRoutes(services: APIServices): Router {
  const router = Router();
  ...
  router.get('/models/:modelId/metrics', async (req: Request, res: Response) => {
    try {
      const { modelId } = req.params;
      const { timeframe = '24h' } = req.query;

      const metrics = await services.aiRoutingService.performanceMonitor.getModelMetrics(
        modelId,
        timeframe as string
      );
  ...
  router.post('/models/:modelId/test', async (req: Request, res: Response) => {
    try {
      const { modelId } = req.params;
      const { testPrompt = 'Hello, this is a connectivity test.' } = req.body;

      const testResult = await services.aiRoutingService.modelRegistry.testModelConnectivity(
        modelId,
        testPrompt
      );
  ...
  router.get('/requests', async (req: Request, res: Response) => {
    try {
      ...
      const requests = Array.from({ length: parseInt(limit as string) }, (_, i) => ({
      ...
      res.json({
        ...
        pagination: {
          ...
          hasMore: parseInt(offset as string) + parseInt(limit as string) < 1000
        },
        filters: {
          userId,
          sessionId,
  ...
  router.post('/recommendations', async (req: Request, res: Response) => {
    try {
      ...
      const recommendations = await services.aiRoutingService.routingEngine.getRoutingRecommendations(
        analysis
      );
