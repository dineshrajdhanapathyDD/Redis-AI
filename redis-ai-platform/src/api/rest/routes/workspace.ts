...
export function workspaceRoutes(services: APIServices): Router {
  const router = Router();
  ...
  // Join workspace
  router.post('/:workspaceId/join', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
      const { userId, role = CollaboratorRole.VIEWER } = req.body;
  ...
  // Leave workspace
  router.post('/:workspaceId/leave', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
  ...
  router.post('/:workspaceId/knowledge', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
      ...
      if (!Object.values(KnowledgeType).includes(type)) {
        return res.status(400).json({
          ...
          message: `Type must be one of: ${Object.values(KnowledgeType).join(', ')}`
        });
      }
  ...
  router.post('/:workspaceId/knowledge/search', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
      ...
      logger.error('Query knowledge error:', error);
      res.status(500).json({
  ...
  router.post('/:workspaceId/knowledge/relationships', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
      ...
      logger.error('Add knowledge relationship error:', error);
      res.status(500).json({
  ...
  router.get('/:workspaceId/insights', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
  ...
  router.get('/:workspaceId/clusters', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
      ...
      const clusters = await services.workspaceService.manager.clusterWorkspaceKnowledge(
        workspaceId,
        algorithm as any
      );
  ...
  router.get('/:workspaceId/metrics', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
  ...
  router.get('/:workspaceId/active-users', async (req: Request, res: Response) => {
    try {
      const { workspaceId } = req.params;
  ...
  router.patch('/:workspaceId/settings', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Update workspace settings error:', error);
      res.status(500).json({
