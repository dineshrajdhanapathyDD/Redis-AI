...
export function searchRoutes(services: APIServices): Router {
  const router = Router();

  // Multi-modal search endpoint
  router.post('/multi-modal', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Multi-modal search error:', error);
      res.status(500).json({
        error: 'Search failed',
  ...
  router.post('/cross-modal', async (req: Request, res: Response) => {
    try {
      ...
      if (!Object.values(ContentType).includes(sourceType)) {
        return res.status(400).json({
          error: 'Invalid source type',
          message: `Source type must be one of: ${Object.values(ContentType).join(', ')}`
        });
      }

      const validTargetTypes = targetTypes.filter((type: string) => 
        Object.values(ContentType).includes(type as ContentType)
      );
      ...
      if (validTargetTypes.length === 0) {
        return res.status(400).json({
          ...
          message: `Target types must include at least one of: ${Object.values(ContentType).join(', ')}`
        });
      }

      // First search in source type
      const sourceResults = await services.multiModalSearch.search(
        query,
        [sourceType],
        Math.min(limit * 2, 20), // Get more source results for cross-modal matching
        threshold
      );
      ...
      for (const sourceResult of sourceResults.slice(0, 5)) { // Limit to top 5 for performance
        const matches = await services.multiModalSearch.search(
          sourceResult.content,
          validTargetTypes,
          limit,
          threshold * 0.8 // Slightly lower threshold for cross-modal
        );
      ...
      logger.error('Cross-modal search error:', error);
      res.status(500).json({
