...
import { User, LoginCredentials, RegisterData } from '../../../services/auth/types';
...
export function createAuthRoutes(
  ...
  router.post('/login',
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!result.success) {
          logger.warn(`Login failed for email: ${credentials.email} - ${result.error}`);
  ...
  // Token refresh
  router.post('/refresh',
    authMiddleware.rateLimit({ maxRequests: 20, windowMs: 15 * 60 * 1000 }),
    async (req: Request, res: Response) => {
      try {
  ...
  router.post('/reset-password',
    ...
    authMiddleware.rateLimit({ maxRequests: 5, windowMs: 15 * 60 * 1000 }),
    async (req: Request, res: Response) => {
      try {
  ...
  router.post('/mfa/verify-setup',
    ...
    async (req: AuthenticatedRequest, res: Response) => {
      try {
        ...
        logger.info(`MFA setup verified for user: ${userId}`);
  ...
  router.put('/admin/users/:userId/roles',
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        logger.info(`User roles updated: ${userId} - roles: ${roles.join(', ')}`);
