import { createAIRoutingService, createAIRequest } from '../services/ai-routing';
...
async function runAIRoutingDemo() {
  try {
    logger.info('Starting Redis AI Platform AI Routing Demo');
    ...
    logger.info('AI Routing Service initialized successfully');

    // Demo 1: Basic request routing
    logger.info('Demo 1: Basic AI request routing');
    
    const basicRequest = createAIRequest(
      'Explain the concept of vector databases and their applications in AI systems',
    ...
    logger.info('Basic request routed successfully:', {
    ...
    logger.info('Demo 2: Code generation request routing');
    
    const codeRequest = createAIRequest(
      `Write a TypeScript function that implements a Redis-based vector similarity search:
    ...
    logger.info('Code generation request routed:', {
    ...
    logger.info('Demo 3: Question answering with user context');
    
    const qaRequest = createAIRequest(
      'What are the key differences between HNSW and FLAT vector indexing algorithms in Redis?',
    ...
    logger.info('Q&A request routed with context:', {
    ...
    logger.info('Demo 4: Batch request processing with different priorities');
    
    const batchRequests = [
      createAIRequest(
        'Summarize the benefits of using Redis for AI applications',
        AIRequestType.SUMMARIZATION,
        { metadata: { priority: 'low' } }
      ),
      createAIRequest(
        'URGENT: Translate this error message to Spanish: "Vector index not found"',
        AIRequestType.TRANSLATION,
        { metadata: { priority: 'high', maxLatency: 1000 } }
      ),
      createAIRequest(
        'Generate a creative story about an AI that learns to paint',
        AIRequestType.TEXT_GENERATION,
        { metadata: { priority: 'medium', requiredCapabilities: ['creativity'] } }
      ),
    ];

    const batchResults = await Promise.all(
      batchRequests.map(request => aiRoutingService.routeRequest(request))
    );

    logger.info('Batch requests processed:', {
    ...
    logger.info('Demo 5: Model performance and health monitoring');
    
    const routingStats = await aiRoutingService.getRoutingStats();
    logger.info('Current routing statistics:', {
    ...
    logger.info('Model health status:', {
      ...
      healthyModels: Object.values(healthStatus).filter((h: any) => h.isHealthy).length,
      modelDetails: Object.entries(healthStatus).map(([modelId, health]: [string, any]) => ({
        modelId,
    ...
    logger.info('Demo 6: Dynamic configuration updates');
    ...
    const costOptimizedRequest = createAIRequest(
      'Generate a brief explanation of machine learning',
      AIRequestType.TEXT_GENERATION
    );

    const costOptimizedResult = await aiRoutingService.routeRequest(costOptimizedRequest);
    logger.info('Cost-optimized routing result:', {
    ...
    const performanceOptimizedRequest = createAIRequest(
      'Generate a brief explanation of machine learning',
      AIRequestType.TEXT_GENERATION,
      { metadata: { maxLatency: 2000 } }
    );

    const performanceOptimizedResult = await aiRoutingService.routeRequest(performanceOptimizedRequest);
    logger.info('Performance-optimized routing result:', {
    ...
    logger.info('Demo 7: Error handling and fallback scenarios');
    
    try {
      // Try to route an unsupported request type
      const unsupportedRequest = createAIRequest(
        'Analyze this image: [image data]',
        AIRequestType.IMAGE_ANALYSIS // No models registered for this type
      );

      await aiRoutingService.routeRequest(unsupportedRequest);
    } catch (error) {
      logger.info('Expected error for unsupported request type:', {
    ...
    logger.info('Demo 8: Model registration and management');
    ...
    try {
      ...
      const customModelRequest = createAIRequest(
        'Test request for custom model',
        AIRequestType.TEXT_GENERATION,
        {
          metadata: {
      ...
      logger.info('Custom model routing result:', {
      ...
      logger.warn('Custom model registration failed:', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }

    // Demo 9: Final statistics and cleanup
    logger.info('Demo 9: Final statistics and performance summary');
    
    const finalStats = await aiRoutingService.getRoutingStats();
    logger.info('Final routing statistics:', {
    ...
    logger.info('Redis AI Platform AI Routing Demo completed successfully!');
    ...
    logger.error('AI Routing demo failed:', {
      error: error instanceof Error ? error.message : 'Unknown error',
