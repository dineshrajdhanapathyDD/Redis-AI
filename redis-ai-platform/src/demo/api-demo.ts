...
import { RestAPI, APIServices, defaultAPIConfig } from '../api/rest';
import { GraphQLAPI, defaultGraphQLConfig } from '../api/graphql';
...
async function runAPIDemo() {
  ...
  try {
    ...
    const graphqlAPI = new GraphQLAPI(redis, services, defaultGraphQLConfig);
    ...
    httpServer.listen(PORT, () => {
      ...
      console.log('\n🔧 Configuration:');
      console.log('=====================================');
      console.log(`• CORS Origins: ${defaultAPIConfig.corsOrigins.join(', ')}`);
      console.log(`• Rate Limit: ${defaultAPIConfig.rateLimitMaxRequests} requests per ${defaultAPIConfig.rateLimitWindowMs / 1000}s`);
      console.log(`• GraphQL Playground: ${defaultGraphQLConfig.playground ? 'Enabled' : 'Disabled'}`);
      console.log(`• GraphQL Introspection: ${defaultGraphQLConfig.introspection ? 'Enabled' : 'Disabled'}`);
