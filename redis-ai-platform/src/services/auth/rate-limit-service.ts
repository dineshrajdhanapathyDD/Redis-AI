...
import { RateLimitConfig, SecurityEvent, SecurityEventType } from './types';
...
export class RateLimitService {
  ...
  async checkDistributedRateLimit(
    ...
    try {
      ...
      logger.error('Error checking distributed rate limit:', error);
      return {
        allowed: true,
        remaining: maxRequests - 1,
        resetTime: Date.now() + windowMs,
   ...
   */
  async getRateLimitInfo(key: string, windowMs: number): Promise<RateLimitInfo> {
    try {
      const now = Date.now();
      const windowStart = now - windowMs;
      const rateLimitKey = `rate_limit:${key}`;
   ...
   */
  async getRateLimitedKeys(pattern: string = '*'): Promise<string[]> {
    try {
      const keys = await this.redis.keys(`rate_limit:${pattern}`);
      return keys.map(key => key.replace('rate_limit:', ''));
    } catch (error) {
      logger.error('Error getting rate limited keys:', error);
      return [];
    }
  }
  ...
  async checkAdaptiveRateLimit(
    ...
    try {
      ...
      logger.error('Error checking adaptive rate limit:', error);
      return {
        allowed: true,
