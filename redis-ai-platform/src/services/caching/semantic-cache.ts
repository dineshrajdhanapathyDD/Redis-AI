...
import { ContentType, VectorEmbedding } from '@/types';
import { createContent } from '../embeddings';
...
export class SemanticCache {
  ...
  private vectorStorage = new VectorStorageService('semantic_cache');
  ...
  constructor(config?: Partial<SemanticCacheConfig>) {
    ...
    logger.info('Semantic cache initialized', { config: this.config });
  }

  async get(query: string, context?: string[]): Promise<CacheHit | null> {
    const startTime = Date.now();

    try {
      // Generate query embedding
      const queryContent = createContent(
        `cache_query_${Date.now()}`,
        ContentType.TEXT,
        query,
        {
          title: 'Cache Query',
      ...
      const similarEntries = await this.vectorStorage.searchSimilarVectors(
      ...
      logger.error('Failed to get from semantic cache', {
        query: query.substring(0, 100),
  ...
  async set(
    ...
    try {
      ...
      const embeddingForStorage: VectorEmbedding = {
        ...queryEmbedding,
        id: cacheEntry.id,
      ...
      logger.error('Failed to set semantic cache', {
        query: query.substring(0, 100),
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }
  ...
  async warmup(queries: string[]): Promise<void> {
    ...
    try {
      for (const query of queries) {
        ...
        const mockMetadata: CacheEntryMetadata = {
          model: 'warmup-model',
          responseTime: 1000,
  ...
  private async getCacheEntry(entryId: string): Promise<CacheEntry | null> {
    try {
      const client = this.redisManager.getClient();
  ...
  private async updateAccessStats(entry: CacheEntry): Promise<void> {
    try {
      ...
      logger.debug('Failed to update access stats', {
        entryId: entry.id,
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }
  ...
  private async evictEntry(entryId: string): Promise<void> {
    try {
      ...
      logger.error('Failed to evict cache entry', {
        entryId,
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }
  ...
  private async evictEntries(count: number): Promise<string[]> {
    try {
      ...
      logger.error('Failed to evict cache entries', {
        count,
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      return [];
  ...
  private async getEvictionCandidates(count: number): Promise<string[]> {
    const client = this.redisManager.getClient();
    const keys = await client.keys('semantic_cache:entry:*');
    ...
    for (const key of keys) {
      const entry = await client.json.get(key) as CacheEntry;
      if (!entry) continue;
  ...
  private async calculateSemanticRelevance(entry: CacheEntry): Promise<number> {
  ...
  private async getRecentQueries(limit: number): Promise<Array<{ query: string; embedding: number[] }>> {
  ...
  private async findExpiredEntries(): Promise<string[]> {
    const client = this.redisManager.getClient();
    const keys = await client.keys('semantic_cache:entry:*');
  ...
  private async compressExistingEntries(): Promise<void> {
  ...
  private isCompressed(response: any): boolean {
  ...
  private startEvictionTimer(): void {
    if (!this.config.enableEviction) return;

    this.evictionTimer = setInterval(async () => {
      try {
        await this.optimize();
      } catch (error) {
        logger.error('Eviction timer failed', {
          error: error instanceof Error ? error.message : 'Unknown error',
        });
      }
