...
export class PerformanceMonitor {
    ...
    try {
      ...
      if (global.gc && typeof global.gc === 'function') {
        ...
        global.gc = (...args: any[]) => {
          ...
          this.gcStats = this.gcStats.filter(stat => 
            Date.now() - stat.timestamp < 3600000 // 1 hour
          );
      ...
      try {
        ...
        logger.warn('Performance hooks not available:', error);
      }
    } catch (error) {
      logger.warn('GC monitoring initialization failed:', error);
    }
  }
  ...
  private collectMetrics(): PerformanceMetrics {
    ...
    const recentGC = this.gcStats.filter(stat => 
      Date.now() - stat.timestamp < 300000 // Last 5 minutes
    );
  ...
  getHistoricalMetrics(durationMs: number = 3600000): PerformanceSnapshot[] {
  ...
  getPerformanceTrends(): any {
    ...
    return {
      ...
      connectionUtilizationTrend: this.calculateTrend(
        olderAvg.connectionPoolUtilization, 
        recentAvg.connectionPoolUtilization
      )
    };
  }
  ...
  private calculateTrend(oldValue: number, newValue: number): 'improving' | 'degrading' | 'stable' {
