...
export class ContentConsistencyService {
  public readonly brandAnalyzer: BrandAnalyzer;
  public readonly contentAdapter: ContentAdapter;
  public readonly performanceTracker: PerformanceTracker;

  constructor(redis: Redis, embeddingManager: EmbeddingManager) {
    this.brandAnalyzer = new BrandAnalyzer(redis, embeddingManager);
    this.contentAdapter = new ContentAdapter(redis, embeddingManager);
    this.performanceTracker = new PerformanceTracker(redis);
  }

  async initialize(): Promise<void> {
    // Initialize content consistency service components
    logger.info('Initializing Content Consistency Service');
    ...
    logger.info('Content Consistency Service initialized successfully');
  }

  async shutdown(): Promise<void> {
    // Cleanup logic when shutting down the service
    logger.info('Shutting down Content Consistency Service');
    
    // Save any pending performance data
    await this.savePendingData();
    
    logger.info('Content Consistency Service shutdown complete');
  }

  private async setupDefaultPlatforms(): Promise<void> {
    // Set up common social media platforms
    const platforms = [
      {
        id: 'twitter',
        name: 'Twitter',
        type: 'social_media' as any,
        characteristics: {
          communicationStyle: 'conversational' as any,
          contentPace: 'fast' as any,
          interactionLevel: 'high' as any,
          visualImportance: 'moderate' as any,
          attentionSpan: 'very_short' as any,
      ...
      },
      {
        ...
        characteristics: {
          ...
          contentPace: 'medium' as any,
          interactionLevel: 'medium' as any,
          visualImportance: 'important' as any,
          attentionSpan: 'medium' as any,
          contentLifespan: 'medium' as any
        },
        constraints: {
          textLimits: {
            maxCharacters: 3000,
    ...
    for (const platform of platforms) {
      await this.contentAdapter.registerPlatform(platform as any);
    }

    logger.info(`Set up ${platforms.length} default platforms`);
  }
