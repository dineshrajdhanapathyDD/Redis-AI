...
exports.AIRoutingHandler = void 0;
...
class AIRoutingHandler {
    ...
    setupHandlers(socket, connectionInfo) {
        ...
        socket.on('ai_routing:subscribe_updates', async (data) => {
            try {
                const { modelIds = [] } = data;
                ...
                await this.redis.hset(`ai_routing_prefs:${socket.id}`, 'modelIds', JSON.stringify(modelIds), 'subscribedAt', Date.now().toString());
                ...
                logger_1.logger.error('Subscribe AI routing updates error:', error);
                socket.emit('error', { message: 'Failed to subscribe to AI routing updates', error: error.message });
            }
        });
        ...
        socket.on('ai_routing:route_request', async (data) => {
            try {
                ...
                logger_1.logger.error('Route AI request error:', error);
                socket.emit('error', { message: 'Failed to route AI request', error: error.message });
            }
        });
        ...
        socket.on('ai_routing:get_model_metrics', async (data) => {
            try {
                ...
                const metrics = await this.services.aiRoutingService.performanceMonitor.getModelMetrics(modelId, timeframe);
                if (!metrics) {
                ...
                socket.emit('error', { message: 'Failed to get model metrics', error: error.message });
            }
        });
        ...
        socket.on('ai_routing:test_model', async (data) => {
            try {
                ...
                const testResult = await this.services.aiRoutingService.modelRegistry.testModelConnectivity(modelId, testPrompt);
                ...
                socket.emit('error', { message: 'Failed to test model connectivity', error: error.message });
            }
        });
        ...
        socket.on('ai_routing:get_analytics', async (data) => {
            try {
                ...
                socket.emit('error', { message: 'Failed to get routing analytics', error: error.message });
            }
        });
        ...
        socket.on('ai_routing:get_recommendations', async (data) => {
            try {
                ...
                const recommendations = await this.services.aiRoutingService.routingEngine.getRoutingRecommendations(analysis);
                ...
                socket.emit('error', { message: 'Failed to get routing recommendations', error: error.message });
            }
        });
        // Subscribe to model status changes
        socket.on('ai_routing:subscribe_model_status', async (data) => {
            try {
                const { modelIds = [] } = data;
                ...
                await this.redis.hset(`ai_routing_model_prefs:${socket.id}`, 'modelIds', JSON.stringify(modelIds), 'subscribedAt', Date.now().toString());
                ...
                logger_1.logger.error('Subscribe model status error:', error);
                socket.emit('error', { message: 'Failed to subscribe to model status', error: error.message });
            }
        });
    }
    // Method to broadcast model status changes
    async broadcastModelStatusChange(modelId, status, metadata) {
        try {
            ...
            logger_1.logger.error('Broadcast model status change error:', error);
        }
    }
    ...
    async broadcastMetricsUpdate(metrics) {
        try {
            ...
            logger_1.logger.error('Broadcast metrics update error:', error);
        }
    }
}
exports.AIRoutingHandler = AIRoutingHandler;
