...
class AuditService {
    ...
    ALERT_KEY_PREFIX = 'security_alert:';
    USER_RISK_KEY_PREFIX = 'user_risk:';
    ...
    async recordEvent(event) {
        try {
            ...
            await this.redis.setex(eventKey, this.RETENTION_DAYS * 24 * 60 * 60, // TTL in seconds
            ...
            logger_1.logger.debug(`Audit event recorded: ${auditEvent.id}`, {
            ...
            logger_1.logger.error('Failed to record audit event:', error);
            throw error;
        }
    }
    // Get audit events
    async getEvents(query = {}) {
        try {
            const { userId, action, resource, startTime = 0, endTime = Date.now(), success, minRiskScore, limit = 100, offset = 0, } = query;
            let eventIds = [];
                ...
                if (result && result[1]) {
                    try {
                        ...
                        if (success !== undefined && event.success !== success)
                            continue;
                        if (minRiskScore !== undefined && (event.riskScore || 0) < minRiskScore)
                            continue;
                        ...
                        logger_1.logger.warn('Failed to parse audit event:', parseError);
                    }
                }
            }
            ...
            logger_1.logger.error('Failed to get audit events:', error);
            throw error;
        }
    }
    ...
    async getEvent(eventId) {
        try {
            ...
            logger_1.logger.error('Failed to get audit event:', error);
            throw error;
        }
    }
    ...
    async createAlert(alert) {
        try {
            ...
            await this.redis.setex(alertKey, this.RETENTION_DAYS * 24 * 60 * 60, JSON.stringify(securityAlert));
            ...
            logger_1.logger.warn(`Security alert created: ${securityAlert.id}`, {
            ...
            for (const result of results || []) {
                if (result && result[1]) {
                    try {
                        ...
                        logger_1.logger.warn('Failed to parse security alert:', parseError);
                    }
                }
            }
            return alerts;
    ...
    async resolveAlert(alertId, resolvedBy) {
        try {
            ...
            await this.redis.setex(alertKey, this.RETENTION_DAYS * 24 * 60 * 60, JSON.stringify(alert));
            // Remove from active alerts
            const activeAlertsKey = `${this.ALERT_KEY_PREFIX}active`;
            await this.redis.zrem(activeAlertsKey, alertId);
            logger_1.logger.info(`Security alert resolved: ${alertId} by ${resolvedBy}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('Failed to resolve security alert:', error);
            throw error;
        }
    }
    // Get user risk score
    async getUserRiskScore(userId) {
        try {
            ...
            logger_1.logger.error('Failed to get user risk score:', error);
            return 0;
        }
    }
    ...
    async updateUserRiskScore(userId, event) {
        try {
            const riskKey = `${this.USER_RISK_KEY_PREFIX}${userId}`;
            ...
            logger_1.logger.error('Failed to update user risk score:', error);
        }
    }
    ...
    async checkForSecurityAlerts(event) {
        try {
            ...
            logger_1.logger.error('Failed to check for security alerts:', error);
        }
    }
}
