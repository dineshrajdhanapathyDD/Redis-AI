...
class AuthService {
    ...
    constructor(redis, config) {
        this.redis = redis;
        this.config = config;
        this.jwtService = new jwt_service_1.JWTService(redis, config);
        this.passwordService = new password_service_1.PasswordService(redis, config);
        this.mfaService = new mfa_service_1.MfaService(redis, config);
        this.rbacService = new rbac_service_1.RBACService(redis);
    }
    /**
    ...
    async initialize() {
        try {
            ...
            logger_1.logger.info('Authentication service initialized');
        }
        catch (error) {
     ...
     * Register a new user
     */
    async register(request, ipAddress, userAgent) {
        try {
            ...
            logger_1.logger.info(`User registered: ${request.email}`);
            return { user, tokens };
        }
        catch (error) {
    ...
    async login(request, ipAddress, userAgent) {
        try {
            ...
            if (!user) {
                await this.logSecurityEvent({
                    type: types_1.SecurityEventType.LOGIN_FAILED,
            ...
            if (await this.isAccountLocked(user.id)) {
                await this.logSecurityEvent({
                    userId: user.id,
                    type: types_1.SecurityEventType.LOGIN_BLOCKED,
            ...
            // Generate tokens
            const tokens = await this.jwtService.generateTokens(user, request.deviceFingerprint, ipAddress, userAgent);
            logger_1.logger.info(`User logged in: ${request.email}`);
            return { user, tokens };
        }
        catch (error) {
     ...
     */
    async refreshTokens(refreshToken, deviceFingerprint, ipAddress, userAgent) {
        try {
            return await this.jwtService.refreshToken(refreshToken, deviceFingerprint, ipAddress, userAgent);
        }
        catch (error) {
            logger_1.logger.error('Error refreshing tokens:', error);
            throw error;
        }
    }
    /**
    ...
    async requestPasswordReset(request) {
        try {
            ...
            if (!user) {
                ...
                logger_1.logger.info(`Password reset requested for non-existent email: ${request.email}`);
                return;
            }
            const token = await this.passwordService.generatePasswordResetToken(user.id, user.email);
            // In a real implementation, you would send an email here
            logger_1.logger.info(`Password reset token generated for user ${user.id}: ${token}`);
        }
        catch (error) {
    ...
    async confirmPasswordReset(request) {
        try {
            ...
            logger_1.logger.info(`Password reset completed for user ${tokenData.userId}`);
        }
        catch (error) {
            logger_1.logger.error('Error confirming password reset:', error);
            throw error;
        }
    }
    /**
    ...
    async changePassword(userId, request) {
        try {
            ...
            await this.logSecurityEvent({
                userId,
                type: types_1.SecurityEventType.PASSWORD_CHANGED,
     ...
     */
    async createAuthContext(token, ipAddress, userAgent) {
        try {
            ...
            logger_1.logger.error('Error creating auth context:', error);
            return null;
        }
    }
    /**
    ...
    async isAccountLocked(userId) {
        try {
            ...
            logger_1.logger.error('Error checking account lock status:', error);
            return false;
        }
    }
    /**
    ...
    async incrementFailedAttempts(userId) {
        try {
            ...
            if (security.failedLoginAttempts >= this.config.security.maxFailedAttempts) {
                ...
                await this.logSecurityEvent({
                    userId,
                    type: types_1.SecurityEventType.ACCOUNT_LOCKED,
    ...
    async resetFailedAttempts(userId) {
        try {
            ...
            logger_1.logger.error('Error resetting failed attempts:', error);
        }
    }
    /**
     * Log security event
