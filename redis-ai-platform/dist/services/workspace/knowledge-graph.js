...
exports.KnowledgeGraph = exports.ChangeType = exports.InsightType = exports.PathType = void 0;
...
(function (PathType) {
    ...
    PathType["TEMPORAL"] = "temporal";
})(PathType || (exports.PathType = PathType = {}));
var InsightType;
(function (InsightType) {
    InsightType["PATTERN_DISCOVERY"] = "pattern_discovery";
    InsightType["KNOWLEDGE_GAP"] = "knowledge_gap";
    InsightType["CONTRADICTION"] = "contradiction";
...
var ChangeType;
(function (ChangeType) {
    ChangeType["NODE_ADDED"] = "node_added";
    ChangeType["NODE_UPDATED"] = "node_updated";
    ChangeType["NODE_DELETED"] = "node_deleted";
    ChangeType["EDGE_ADDED"] = "edge_added";
    ChangeType["EDGE_UPDATED"] = "edge_updated";
    ChangeType["EDGE_DELETED"] = "edge_deleted";
    ChangeType["RELATIONSHIP_STRENGTHENED"] = "relationship_strengthened";
    ChangeType["RELATIONSHIP_WEAKENED"] = "relationship_weakened";
...
class KnowledgeGraph {
    ...
    NODE_PREFIX = 'kg_node';
    EDGE_PREFIX = 'kg_edge';
    CLUSTER_PREFIX = 'kg_cluster';
    INSIGHT_PREFIX = 'kg_insight';
    EVOLUTION_PREFIX = 'kg_evolution';
    ...
    async addNode(workspaceId, node) {
        ...
        await this.redis.hset(nodeKey, 'id', nodeId, 'type', node.type, 'content', node.content, 'embeddings', Buffer.from(new Float32Array(embeddings).buffer), 'metadata', JSON.stringify(node.metadata), 'createdBy', node.createdBy, 'createdAt', knowledgeNode.createdAt.toISOString());
        ...
        logger_1.logger.info(`Added knowledge node ${nodeId} to workspace ${workspaceId}`);
        return knowledgeNode;
    }
    async addEdge(workspaceId, edge) {
        ...
        await this.redis.hset(edgeKey, 'id', edgeId, 'sourceId', edge.sourceId, 'targetId', edge.targetId, 'relationship', edge.relationship, 'strength', edge.strength.toString(), 'createdAt', knowledgeEdge.createdAt.toISOString());
        ...
        logger_1.logger.info(`Added knowledge edge ${edgeId} to workspace ${workspaceId}`);
        return knowledgeEdge;
    }
    async queryGraph(workspaceId, query) {
    ...
    async discoverInsights(workspaceId) {
        const insights = [];
        // Pattern discovery
    ...
    async clusterKnowledge(workspaceId, algorithm = 'community') {
    ...
    async getGraphMetrics(workspaceId) {
        const nodes = await this.getAllNodes(workspaceId);
    ...
    async getEvolutionHistory(workspaceId, limit = 100) {
    ...
    async findSimilarNodes(workspaceId, queryEmbedding, nodeTypes, limit = 20) {
        ...
        try {
            ...
            const results = await this.redis.call('FT.SEARCH', indexName, searchQuery, 'PARAMS', '2', 'query_vec', Buffer.from(new Float32Array(queryEmbedding).buffer), 'SORTBY', 'score', 'RETURN', '7', 'id', 'type', 'content', 'metadata', 'createdBy', 'createdAt', 'score', 'DIALECT', '2');
            return this.parseNodeSearchResults(results);
        }
        catch (error) {
            logger_1.logger.error(`Node similarity search failed: ${error.message}`);
            return [];
        }
    }
    async findRelatedEdges(workspaceId, nodeIds, relationshipTypes) {
        const edges = [];
    ...
    async findKnowledgePaths(workspaceId, nodes, maxDepth) {
        const paths = [];
        ...
        for (let i = 0; i < topNodes.length; i++) {
            for (let j = i + 1; j < topNodes.length; j++) {
                const pathsFound = await this.findPathsBetweenNodes(workspaceId, topNodes[i].id, topNodes[j].id, maxDepth);
                paths.push(...pathsFound);
            }
        }
        return paths.sort((a, b) => b.score - a.score);
    }
    async findPathsBetweenNodes(workspaceId, sourceId, targetId, maxDepth) {
        const paths = [];
    ...
    async discoverRelationships(workspaceId, newNode) {
    ...
    async inferRelationshipType(node1, node2) {
        ...
        if (content1.includes('because') || content1.includes('therefore') ||
            ...
            return workspace_manager_1.RelationshipType.DEPENDS_ON;
        }
        // Check for contradictions
        if ((content1.includes('not') && content2.includes('is')) ||
            (content1.includes('false') && content2.includes('true'))) {
            return workspace_manager_1.RelationshipType.CONTRADICTS;
        }
        ...
        if (content1.includes('supports') || content1.includes('confirms') ||
            ...
            return workspace_manager_1.RelationshipType.SUPPORTS;
        }
        // Check for references
        if (content1.includes('see') || content1.includes('refer') ||
            content2.includes('see') || content2.includes('refer')) {
            return workspace_manager_1.RelationshipType.REFERENCES;
        }
        // Default to general relationship
        return workspace_manager_1.RelationshipType.RELATES_TO;
    }
    async discoverPatterns(workspaceId) {
        const insights = [];
        const nodes = await this.getAllNodes(workspaceId);
    ...
    async identifyKnowledgeGaps(workspaceId) {
        const insights = [];
        const nodes = await this.getAllNodes(workspaceId);
        const edges = await this.getAllEdges(workspaceId);
    ...
    async detectContradictions(workspaceId) {
        const insights = [];
        const edges = await this.getAllEdges(workspaceId);
        // Find contradiction relationships
        const contradictionEdges = edges.filter(edge => edge.relationship === workspace_manager_1.RelationshipType.CONTRADICTS);
    ...
    async analyzeTrends(workspaceId) {
        const insights = [];
        const evolutions = await this.getEvolutionHistory(workspaceId, 100);
        // Analyze recent activity patterns
        const recentEvolutions = evolutions.filter(e => Date.now() - e.timestamp.getTime() < 7 * 24 * 60 * 60 * 1000 // Last 7 days
        );
    ...
    async communityDetection(workspaceId, nodes) {
    ...
    async kMeansClustering(nodes, k = 3) {
        if (nodes.length < k) {
    ...
    async hierarchicalClustering(nodes) {
    ...
    calculateCoherenceScore(cluster) {
        if (cluster.length < 2)
            return 1.0;
        let totalSimilarity = 0;
        let count = 0;
    ...
    async expandCluster(startNode, allNodes, adjacencyMatrix, visited) {
    ...
    calculatePathScore(nodes, edges) {
        if (edges.length === 0)
            return 0;
    ...
    async calculateClusteringCoefficient(workspaceId, nodes, edges) {
        if (nodes.length < 3)
            return 0;
        ...
        for (const node of nodes) {
            ...
            for (let i = 0; i < neighbors.length; i++) {
                for (let j = i + 1; j < neighbors.length; j++) {
                    const edgeExists = edges.some(edge => (edge.sourceId === neighbors[i] && edge.targetId === neighbors[j]) ||
    ...
    async calculateCentralityScores(workspaceId, nodes, edges) {
    ...
    async getNodeById(workspaceId, nodeId) {
    ...
    async getNodesByIds(workspaceId, nodeIds) {
        const nodes = [];
        for (const nodeId of nodeIds) {
    ...
    async getEdgesForPath(workspaceId, nodePath) {
        const edges = [];
    ...
    async getAdjacentNodes(workspaceId, nodeId) {
        ...
        for (const edgeKey of [...outgoing, ...incoming]) {
            const edgeData = await this.redis.hgetall(edgeKey);
            if (edgeData) {
                if (edgeData.sourceId !== nodeId)
                    adjacentIds.add(edgeData.sourceId);
                if (edgeData.targetId !== nodeId)
                    adjacentIds.add(edgeData.targetId);
            }
        }
    ...
    parseNodeSearchResults(results) {
        const nodes = [];
    ...
    async updateAdjacencyLists(workspaceId, sourceId, targetId, edgeId) {
        const edgeKey = `${this.EDGE_PREFIX}:${workspaceId}:${edgeId}`;
    ...
    async updateGraphStructure(workspaceId, changeType, elementId) {
    ...
    async recordEvolution(workspaceId, evolution) {
    ...
    async calculateEvolutionImpact(workspaceId, changeType, elementId) {
    ...
    async storeInsight(workspaceId, insight) {
    ...
    async storeCluster(workspaceId, cluster) {
