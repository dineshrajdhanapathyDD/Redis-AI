...
const caching_1 = require("../services/caching");
...
async function runCachingDemo() {
    try {
        logger_1.default.info('Starting Redis AI Platform Semantic Caching Demo');
        ...
        const cachingService = (0, caching_1.createCachingService)({
        ...
        logger_1.default.info('Caching system initialized successfully');
        // Demo 1: Basic caching and retrieval
        logger_1.default.info('Demo 1: Basic caching and retrieval');
        const basicRequest = (0, caching_1.createCacheableRequest)('What is Redis and how is it used in AI applications?', types_1.AIRequestType.QUESTION_ANSWERING);
        ...
        logger_1.default.info('Response cached successfully');
        // Second request - cache hit
        logger_1.default.info('Second request (cache hit):');
        ...
        logger_1.default.info('Demo 2: Semantic similarity matching');
        ...
        for (const [index, query] of similarQueries.entries()) {
            const similarRequest = (0, caching_1.createCacheableRequest)(query, types_1.AIRequestType.QUESTION_ANSWERING);
            const startTime = Date.now();
            const result = await cachingService.getCachedResponse(similarRequest);
            const queryTime = Date.now() - startTime;
            logger_1.default.info(`Similar query ${index + 1}:`, {
        ...
        logger_1.default.info('Demo 3: Different request types and caching');
        const requestTypes = [
            {
                type: types_1.AIRequestType.TEXT_GENERATION,
            ...
            {
                type: types_1.AIRequestType.CODE_GENERATION,
...
store_vector('my_vector', vector)`,
            ...
            {
                type: types_1.AIRequestType.SUMMARIZATION,
        ...
        for (const requestData of requestTypes) {
            const request = (0, caching_1.createCacheableRequest)(requestData.query, requestData.type);
            // Cache the response
            ...
            logger_1.default.info(`${requestData.type} request:`, {
        ...
        // Demo 4: Cache warmup
        logger_1.default.info('Demo 4: Cache warmup with common queries');
        const warmupQueries = [
            {
                query: 'What is machine learning?',
                type: types_1.AIRequestType.QUESTION_ANSWERING,
                model: 'gpt-3.5-turbo',
            },
            {
        ...
        logger_1.default.info(`Cache warmed up with ${warmupQueries.length} queries`);
        // Verify warmup worked
        for (const warmupQuery of warmupQueries.slice(0, 2)) {
            const request = (0, caching_1.createCacheableRequest)(warmupQuery.query, warmupQuery.type);
            const result = await cachingService.getCachedResponse(request, warmupQuery.model);
            logger_1.default.info(`Warmup verification:`, {
        ...
        logger_1.default.info('Demo 5: Cache statistics and performance analysis');
        const stats = await cachingService.getCacheStats();
        logger_1.default.info('Cache statistics:', {
        ...
        logger_1.default.info('Demo 6: Cache optimization');
        const optimizationResult = await cachingService.optimizeCache();
        logger_1.default.info('Cache optimization results:', {
        ...
        logger_1.default.info('Demo 7: Query normalization effects');
        ...
        const firstNormalizedRequest = (0, caching_1.createCacheableRequest)(normalizedQueries[0], types_1.AIRequestType.QUESTION_ANSWERING);
        ...
        for (const [index, query] of normalizedQueries.entries()) {
            const request = (0, caching_1.createCacheableRequest)(query, types_1.AIRequestType.QUESTION_ANSWERING);
            const result = await cachingService.getCachedResponse(request);
            logger_1.default.info(`Normalized query ${index + 1}:`, {
                query: `"${query}"`,
                hit: result.hit,
                similarity: result.similarity?.toFixed(3),
                source: result.source,
        ...
        logger_1.default.info('Demo 8: Cache invalidation');
        ...
        for (const query of invalidationQueries) {
            const request = (0, caching_1.createCacheableRequest)(query, types_1.AIRequestType.TEXT_GENERATION);
            const response = {
        ...
        logger_1.default.info(`Invalidated ${invalidatedCount} Redis-related cache entries`);
        // Verify invalidation
        for (const query of invalidationQueries) {
            const request = (0, caching_1.createCacheableRequest)(query, types_1.AIRequestType.TEXT_GENERATION);
            const result = await cachingService.getCachedResponse(request);
            logger_1.default.info(`Post-invalidation check:`, {
        ...
        logger_1.default.info('Demo 9: Performance comparison (cached vs uncached)');
        const performanceTestQuery = 'Explain the architecture of modern AI systems';
        const performanceRequest = (0, caching_1.createCacheableRequest)(performanceTestQuery, types_1.AIRequestType.TEXT_GENERATION);
        // Simulate uncached response time
        const uncachedStartTime = Date.now();
        await new Promise(resolve => setTimeout(resolve, 2500)); // Simulate AI model latency
        ...
        logger_1.default.info('Performance comparison:', {
        ...
        logger_1.default.info('Demo 10: Final cache statistics');
        const finalStats = await cachingService.getCacheStats();
        logger_1.default.info('Final cache performance:', {
        ...
        logger_1.default.info('Redis AI Platform Semantic Caching Demo completed successfully!');
        // Cleanup
        await cachingService.cleanup();
        await redisManager.disconnect();
    }
    catch (error) {
        logger_1.default.error('Caching demo failed:', {
